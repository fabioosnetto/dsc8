...
async function convert() {
   ...
   setTimeout(async () => {
      ...
      pasInput = el_1.value;
      dfmInput = el_2.value;
      fastConversion = el_5.checked;


      // format .dfm and .ps to javascript object
      console.log('Formating DFM and PAS to Javascript...');
      console.log('Result:')
      const dfm_js = dfmToJs(dfmInput);
      const pas_js = pasToJs(pasInput);
      dfmForm = dfm_js[Object.keys(dfm_js)[0]]['_objectClass'];
      console.log(dfm_js);
      console.log(pas_js);
      ...
      // apply styles
      applyStyles(dfm_js, pas_js);
      addHeader(dfm_js, pas_js);
      addButtons(dfm_js, pas_js);
      addTabsButtons(dfm_js, pas_js);
      loading(false);
      if(!fastConversion) await semiAutoConversion(dfm_js)
      
      // provide result
      dfmOutput = jsToDfm(dfm_js);
      pasOutput = jsToPAS(pas_js);

      const el_3 = document.querySelector('#pas-output');
      const el_4 = document.querySelector('#dfm-output');

      if (!el_3 || !el_4) return;
      el_3.value = `${pasOutput}`;
      el_4.value = `${dfmOutput}`;
   }, 0);
}

//--- DFM to Javascript
function dfmToJs(dfmText) {
   const lines = dfmText
   .trim().split('\n')
   ...
   const stack = [];
   let root = {};
   let multilineStyle = { active: false, type: '', level: null, key: '', key1: ''};

   let levels = [];
   lines.map(l => {
      const i1 = l.search(/\S/);
      if(!levels.filter(v => v.i1 === i1).length) {
         levels.push({
            i1: i1,
            i2: levels.reverse().map((v, i) => (i1 < v.i1) ? i+1 : null).filter(v => v)[0] || levels.length
         });
      }
   ...
   for (let [index, line] of lines.entries()) {
      ...
      const level = levels.filter(v => v.i1 === line.search(/\S/))[0].i2; // Finds the first non-whitespace character index
...
//--- Javascript to DFM
function jsToDfm(jsObj) {

   const sortJs = (jsObj) => {
      // Convert object properties to an array of key-value pairs
      const sortedEntries = Object.entries(jsObj).sort((a, b) => {
         const orderA = a[1]?.['_order'] !== undefined ? Number(a[1]?.['_order']) : Number.MAX_SAFE_INTEGER;
         const orderB = b[1]?.['_order'] !== undefined ? Number(b[1]?.['_order']) : Number.MAX_SAFE_INTEGER;
   ...
   const getDfmAsLines = (jsObj, indentLevel) => {
   ...
   const reorderDfm = (lines) => {
      ...
      // push properties
      const pushProperties = (linesStack, indentLevel = 0) => {
      ...
      // push objects
      const pushObjects = (linesStack, indentLevel = 0) => {
...
//--- Javascript to PAS
function jsToPAS(jsText) {
   const unitKey = Object.keys(jsText)[0];
   return jsText[unitKey];
}

//--- PAS Add Object
function pasAddObject(pasObj, objClass, objName) {

   const getObjectsLastLineIndex = (lines) => {
      // get level accordingly to the line indentation level
      let levels = [];
      lines.forEach(l => {
         const i1 = l.search(/\S/);
   ...
   const objIndex = getObjectsLastLineIndex(lines);
...
//--- PAS Remove Object
function pasRemoveObject(pasObj, objClass, objName) {
...
//--- PAS Add Method
function pasAddMethod(pasObj, methodType, methodName, methodParams, methodContent) {

   const getFormName = (lines) => {
   ...
   const getMethodLastLineIndex = (lines) => {
      let formDeclaration = false;
      let formDeclarationLevel = null;
      for (const [index, line] of lines.entries()) {
         const trimmedLine = line.trim();
   ...
   const getMethodDeclarationLastLineIndex = (lines) => {
      let methodMatches = 0;
...
//--- PAS Remove Method
function pasRemoveMethod(pasObj, methodType, methodName) {
...
function extractObjectByClass(obj, className) {
   let result = [];
   ...
   for (const key in obj) {
      if(typeof obj[key] === 'object' && obj[key] !== null) {
         result = result.concat(extractObjectByClass(obj[key], className));
      }
   }
...
//--- Add Buttons
function addButtons(dfmObj, pasObj, parent) {
   for (const key in dfmObj) {
      if(typeof dfmObj[key] === 'object' && dfmObj[key] !== null) {
         if(!dfmObj[key]['_objectClass']) continue;
         addButtons(dfmObj[key], pasObj, dfmObj['_objectClass']);
         ...
         if(String(dfmObj[key]['_objectClass']).includes('TSpeedButton') && dfmObj['_objectClass'] === 'TPanel') {
            ...
            // add button panel
            pasAddObject(pasObj, 'TPanel', `Panel${key}`);
            ...
            pasRemoveObject(pasObj, dfmObj[key]['_objectClass'], key);
            pasAddObject(pasObj, 'TSpeedButton', key);
...
//--- Add Tabs Buttons
function addTabsButtons(dfmObj, pasObj) {
   for (const key in dfmObj) {
      if(typeof dfmObj[key] === 'object' && dfmObj[key] !== null) {
         if(!dfmObj[key]['_objectClass']) continue;
         addTabsButtons(dfmObj[key], pasObj);
         ...
         pasAddObject(pasObj, 'TPanel', `PnlTabs${key}`);
         dfmObj[`PnlTabs${key}`] = {};
         ...
         tabSheets.forEach((tk, i) => {
            ...
            pasAddObject(pasObj, 'TPanel', `Pnl${tk}`);
            ...
            console.log(dfmObj[key][tk])
            dfmObj[`PnlTabs${key}`][`Pnl${tk}`]['TabOrder'] = `${dfmObj[key][tk]['ImageIndex'] || 0}`;

            // add active marker
            pasAddObject(pasObj, 'TPanel', `PnlMarker${tk}`);
            ...
            // add button
            pasAddObject(pasObj, 'TSpeedButton', `Btn${tk}`);
...
async function semiAutoConversion(dfmObj) {
   await semiAutoFieldsAndLabels(dfmObj);
   await semiAutoButtons(dfmObj);
}
async function semiAutoFieldsAndLabels(dfmObj) {
   const getOrders = (obj, className) => {
   ...
   const isLabel = (className) => {
      const classes = ['TLabel'];
      return classes.includes(className);
   };
   const isField = (className) => {
      const classes = ['TEdit', 'TMSEdit'];
      return classes.includes(className);
   };

   const setTopAndLeft = (obj) => {
   ...
   const ask = async (obj, parent = null, startIndex = null) => {
      ...
      for (const key in obj) {
         ...
         if(!parentEl || !classEl || !nameEl || !orderEl || !widthEl || !prevBtn || !nextBtn) break;
         ...
         // set user values
         obj[key]['Width'] = Number(widthEl.value);
         obj[key]['_order'] = orderEl.value;
         if(!isLabel(obj[key]['_objectClass'])) obj[key]['TabOrder'] = orderEl.value;
...
async function semiAutoButtons(dfmObj) {
   const getOrders = (obj, className) => {
      const tabOrders = Object
      .keys(obj)
      ?.filter(k => obj[k]['_objectClass'] && (
   ...
   const isButton = (className) => {
      const classes = ['TSpeedButton'];
      return classes.includes(className);
   };
   const isButtonPanel = (obj) => {
   ...
   const ask = async (obj, parent = null, startIndex = null) => {
      ...
      for (const key in obj) {
         ...
         // set user values
         obj[key]['_order'] = orderEl.value;
         obj[key]['TabOrder'] = orderEl.value;
...
function copyPasOutputToClipboard() {
   const el_1 = document.querySelector('#pas-output');
   if(!el_1) return;

   copyToClipboard(el_1.value);
}
//--- Copy .DFM Output to Clipboard
function copyDfmOutputToClipboard() {
...
object FRomExped: TFRomExped
  ...
  object QryCons: TQuery
   ...
   el_1.value = defPasInput.trim();
   el_2.value = defDfmInput.trim();
   if(fastConversion) el_3.checked;
}
setTimeout(function () { defSettings(); }, 1000);